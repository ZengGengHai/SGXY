...
Page({
  ...
  requestData: function () {
    ...
    if (loading) {   //没次加载完才触发
      if (next) {
        ...
        Product.setQuery(query1).orderBy('-created_at').limit(limit).offset(offset).expand( ['image_id','focus_id']).find().then(res => {
          console.log(res)
          ...
          for (var i = 0; i < res.data.objects.length; i++) {
            ...
            if(res.data.objects[i].image_id.user_name==='全媒体中心'){
              var user_name='全媒体中心'
              var user_image = "https://cloud-minapp-13676.cloud.ifanrusercontent.com/1fnj61GhuZCVSpDa.jpg"
            }else{
              var user_name = res.data.objects[i].focus_id.nickname
              var user_image = res.data.objects[i].focus_id.avatar
            }
            mUrl.push({
              ImageUrl: res.data.objects[i].image_id.ImageUrl, category: res.data.objects[i].image_id.category, user_name, user_image, id: res.data.objects[i].image_id.id, created_at: util.js_date_time(res.data.objects[i].image_id.created_at), watermark: res.data.objects[i].image_id.watermark, Student_ID: res.data.objects[i].image_id.Student_ID, created_by: res.data.objects[i].image_id.created_by,shoucangid: res.data.objects[i].id, content_num: res.data.objects[i].image_id.content_num, Collection_quantity: res.data.objects[i].image_id.Collection_quantity, content: res.data.objects[i].image_id.content, longitude, latitude
            })
            this.setData({
              mUrl: mUrl
            })
  ...
  image_details: function (e) {
    ...
    console.log(image_user_image,'jkjkjkjkj')
    ...
    var shoucangid = e.target.dataset.shoucangid
  ...
  give_up: function () {
    var that = this;
    var shoucangid = this.data.shoucangid;
    var image_id = this.data.image_id;
